...
def inference(bs, sketch, reference, pad_reference, pad_reference_scale, gs_r, gs_s, ctl_scale, strength,
              height, width, seed, low_vram, step, posemb, injection, infid_x, infid_r, injstep, crop,
              start_step, end_step, no_start_step, no_end_step, return_inter, sampler, scheduler, preprocess,
              deterministic, text, target, anchor, control, target_scale, ts0, ts1, ts2, ts3, enhance,
...
def init_inerface(share=False, *args, **kwargs) -> None:
    sampler_list = get_sampler_list() + ["dpm_vp", "ddim"]
    ...
    with gr.Blocks(title="Colorize Diffusion", css="div.gradio-container{ max-width: unset !important; }") as block:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("Advanced Setting", open=True):
                    with gr.Row():
                        ...
                        posemb = gr.Checkbox(label="Activate postive embedding", value=False)
                    with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    sampler = gr.Dropdown(choices=sampler_list, value=sampler_list[0], label="Sampler", scale=2)
        ...
        ips = [bs, sketch_img, reference_img, pad_reference, pad_reference_scale, gs_r, gs_s, ctl_scale,
               strength, height, width, seed, save_memory, step, posemb, injection, injection_control_scale, injection_fidelity,
               injection_start_step, crop, start_step, end_step, no_start_step, no_end_step, return_inter, sampler, scheduler,
               preprocessor, deterministic, text_prompt, target, anchor, control, target_scale, ts0, ts1, ts2, ts3,
               enhance, enc_scale, middle_scale, low_scale, accurate, attn0, attn1, attn2, attn3,
